        ...
        vms = {
          "${i + 1}" = {
            tags   = join(",", [
              ...
              "supernode:${try(vm_group.supernode, can(regex("(super|mev|bootnode)", vm_group.name))) ? "True" : "False"}",
            ])
...
resource "local_file" "ansible_inventory" {
      ...
      hosts = merge(
        {
          for key, server in digitalocean_droplet.main : "do.${key}" => {
            ...
            bandwidth       = try(split(":", tolist(server.tags)[1])[1], 0)
  ...
  filename = "../../ansible/inventories/devnet-ssl-part-msg-3/inventory.ini"
}
