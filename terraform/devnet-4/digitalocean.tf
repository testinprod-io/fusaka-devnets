...
resource "cloudflare_record" "server_record_v4" {
  for_each = {
    for vm in local.digitalocean_vms : "${vm.id}" => vm if can(regex("-[123]$", vm.id))
  }
...
resource "local_file" "ansible_inventory" {
      ...
      hosts = merge(
        {
          for key, server in digitalocean_droplet.main : "do.${key}" => {
            ...
            validator_start = try(split(":", tolist(server.tags)[6])[1], 0)
            validator_end   = try(split(":", tolist(server.tags)[5])[1], 0) # if the tag is not a number it will be 0 - e.g no validator keys
            bandwidth       = try(split(":", tolist(server.tags)[2])[1], 0)
            supernode       = try(title(split(":", tolist(server.tags)[4])[1]), "True")
            tags            = "${server.tags}"
  ...
  filename = "../../ansible/inventories/devnet-4/inventory.ini"
}
