    ...
    [
      for i in range(0, vm_group.count) : {
        ...
        vms = {
          "${i + 1}" = {
            tags   = join(",", [
              "bandwidth:${try(vm_group.bandwidth, 0)}",
              "group_name:${vm_group.name}",
              ...
              "supernode:${"False"}",
              "up_bandwidth:${try(vm_group.up_bandwidth, 0)}",
              "down_bandwidth:${try(vm_group.down_bandwidth, 0)}"
            ])
  ...
  digitalocean_vms = flatten([
    for group in local.digitalocean_vm_groups : [
      for vm_key, vm in group.vms : {
        ...
        bandwidth    = try(vm.bandwidth, 0)
        up_bandwidth    = try(vm.up_bandwidth, 0)
        down_bandwidth    = try(vm.down_bandwidth, 0)
...
resource "local_file" "ansible_inventory" {
      ...
      hosts = merge(
        {
          for key, server in digitalocean_droplet.main : "do.${key}" => {
            ...
            group           = try(split(":", tolist(server.tags)[4])[1], "unknown")
            validator_start = try(split(":", tolist(server.tags)[8])[1], 0)
            validator_end   = try(split(":", tolist(server.tags)[7])[1], 0) # if the tag is not a number it will be 0 - e.g no validator keys
            bandwidth       = try(split(":", tolist(server.tags)[2])[1], 0)
            up_bandwidth       = try(split(":", tolist(server.tags)[6])[1], 0)
            down_bandwidth       = try(split(":", tolist(server.tags)[3])[1], 0)
            supernode       = try(title(split(":", tolist(server.tags)[5])[1]), "False")
            tags            = "${server.tags}"
  ...
  filename = "../../ansible/inventories/devnet-ssl-perf-peerdas-2/inventory.ini"
}
